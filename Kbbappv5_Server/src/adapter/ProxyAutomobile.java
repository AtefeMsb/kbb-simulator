...
public abstract class ProxyAutomobile {
	...
	// build and add
	public void builAutoFromPropertiesObject(Properties props) {
		FileIO fileIO = new FileIO();
		Automobile auto = null;
		auto = fileIO.buildAutoObjectFromPropertiesObject(props);
		autoList.put(auto.createAutoKey(), auto);
	}
	...
	public String showList() {
	  ...
	  StringBuilder AutoModelNames = new StringBuilder();
	  // Get a set of the entries
	  Set set = autoList.entrySet(); 
	  // Get an iterator
	  Iterator i = set.iterator();
	  
	  // save all the model name in an string
	  while(i.hasNext()) {
	     Map.Entry<String,Automobile> entry = (Map.Entry)i.next();
	     Automobile auto = entry.getValue();
	     AutoModelNames.append(auto.getModel()).append(" -- ") ;
	  }

		return AutoModelNames.toString();
	}
